...
# Fall of Delta Green 提取卡住排查

- 目标：检查 375 页 PDF 普通 HTML/Word 翻译卡在“提取”的原因。
- 已确认：审计文件停在 running，progress 里完成页为空，说明还没进入翻译。
- 已确认：PDF 正文提取正常，375 页约 120 秒；真正慢点是普通输出额外调用纯重绘语义分析。
- 已修复：普通输出做语义页型分析时不再保存图片资源；纯重绘 PDF 仍默认保存图片。
- 结果：同一份 375 页 PDF 的版面上下文分析从超过 5 分钟未完成降到约 19 秒。
- 已验证：相关测试 14 个通过；全量测试 465 个通过。

# Fall of Delta Green 导出排版检查

- 目标：对比原 PDF 与最新导出，检查换行和表格是否正常。
- 已确认：旧导出 PDF 有明显问题，`| --- |` 被渲染成红色标题，`| 提示 |` 残留在正文里；部分普通卡片被误生成为大表格。
- 已修复：HTML/Word 只有遇到严格 Markdown 表格分隔行时才生成表格；单格竖线残片会清理为普通文字或丢弃分隔线。
- 已生成：离线重排新版 `output/Fall_of_Delta_Green_final_cn/Fall_of_Delta_Green_final_cn_layoutfix/`，未调用翻译 API。
- 已验证：新版临时 PDF 中 `特训课程` 和 `避免沦为轨道式游戏` 页面不再出现竖线残留；真实表格仍保留。
- 已验证：旧 Word 有 4 处 `| --- |`，新版为 0；全量测试 467 个通过。
- 注意：原 `Fall_of_Delta_Green_final_cn.docx` 当时被占用，未覆盖；使用 `_layoutfix` 目录里的新版文件。

# HTML 转 PDF 页面尺寸修复

- 目标：修复 HTML 打印成 PDF 后白纸套米色页、页面大小看起来不一致的问题。
- 已确认：PDF 本身是 Letter 尺寸；问题来自打印态 `.sheet` 固定成一张假纸，浏览器再套真实纸张。
- 已修复：打印态让真实 PDF 页面承载米色纸底，`.sheet` 不再固定宽度套页；保留最小页高。
- 已生成：`output/Fall_of_Delta_Green_final_cn/Fall_of_Delta_Green_final_cn_layoutfix/Fall_of_Delta_Green_final_cn_layoutfix_print.pdf`。
- 已验证：PDF 尺寸为 612x792 pt；外层白底消失；相关测试 38 个通过。

# KULT La Cena PDF 适配检查

- 目标：检查 `uploads/La Cena.pdf` 是否适合现有翻译和纯重绘排版链路。
- 已确认：文件共 15 页，带真实文字层，不是扫描版；第 1-8 页主要是封面、正文、双栏和人物说明，第 9-15 页是 KULT 角色表。
- 已暴露：现有纯重绘 Phase A 默认提取图片时会因零宽/空图片对象失败，错误为 `cannot write empty image`，成品链路当前不能直接跑完。
- 已确认：跳过图片仅做诊断时，正文页能识别为单栏/双栏/混合页；角色表页有 70 左右文本区和 216 以上矢量线，自动重排会把表格/属性图误当普通正文块。
- 结论：第 1-8 页可沿用现有能力，但需要先修图片提取；第 9-15 页不适合普通自动重排，建议独立按“角色表/模板页”处理，或明确跳过表格区只翻译说明框。

# KULT 角色表保框翻译

- 目标：确认 `La Cena.pdf` 第 9-15 页能否保留角色表框体，只替换文字。
- 已完成：零面积/裁剪后空图片对象会跳过，避免 Phase A 在 KULT 图片对象上崩溃。
- 已完成：密集线框页识别放宽到深色或未标色长线；这类页不再渲染不稳定图片层，改为保留矢量框线和固定框内译文。
- 已完成：密集线框页的水平文本使用译文；旋转文本继续保留原文，避免破坏旧的倾斜/旋转页规则。
- 已完成：纯符号、数字、私有字体复选框不再进入翻译，避免角色表小格溢出。
- 已生成：保框样张 `output/_la_cena_sheet_keep_boxes_check/la_cena_sheet_keep_boxes_check.pdf`，第 9-15 页 7 页，溢出 0，中文可正常渲染。
- 已验证：全量测试 474 个通过。
- 关键决定：KULT 角色表可以独立走“矢量框线 + 固定框译文”路径；不复用普通正文重排，也不渲染 PDF 拆出的图片层。

# The Laraine Estate 漏段检查
- 目标：检查 upload 源 PDF 与 output 里的 HTML/Word 是否存在漏段。
- 已确认：源 PDF 第 3 页提取完整，约 4403 个非空字符；缓存译文只有约 310 个非空字符，停在“如果你想”。
- 已确认：现有 HTML 和 Word 在第 3 页后半段缺正文，并直接接到下一页内容；其它页未发现同类长度异常。
- 已处理：新增长正文页“译文明显过短”拦截；Web 和 CLI 导出前都会把疑似截断页标为失败页，不再输出成完成品。
- 已验证：The Laraine Estate 现有进度只会标出第 3 页；相关测试 74 个通过；语法检查通过。
- 关键决定：现有 The_Laraine_Estate_cn 成品不完整，需要重翻第 3 页后重新导出。

# The Laraine Estate 漏段检查补充
- 已处理：疑似未翻译或截断的译文会同步清理相同内容的翻译缓存，避免重试时继续复用坏结果。
- 已验证：相关测试 91 个通过；代码语法检查通过；现有 The Laraine Estate 进度仍只标出第 3 页异常。

# The Laraine Estate 当前进度处理
- 已处理：`The_Laraine_Estate_cn.progress.json` 中第 3 页已从完成页改为失败页。
- 已处理：同内容坏译文缓存删除 1 条，后续重试不会复用截断译文。
- 未处理：现有 HTML/Word 成品未覆盖，仍需重翻第 3 页后重新导出。
