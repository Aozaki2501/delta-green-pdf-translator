...
第 4 页那 30 多行正文全走这条规则。

**修复。** 定位正文行高提高到 1.4；定位标题改为 1.2 并允许换行。空间不足时由现有导出检查报错，不缩字或裁字。

### T5（P2）一致性问题 —— ✅ 已修复（2026-07-28）

- **`ETERNAL:` 未翻译**：未知全大写正文标签若原样出现在译文中，会作为翻译失败重试；STR、SAN 等明确保留缩写不受影响，旧缓存同样不会复用。
- **同一个「格赫罗斯」标题两种字体**：标题现在只按语义字体角色选择字体，不再让原 PDF 的不同字体覆盖标题样式。
- **续段缩进错位**：T1 已把跨区域续段合并为一个语义块；渲染时只根据该语义块的首行缩进决定样式，不再把后半句作为新段。

### T6（P2）产物卫生 —— ✅ 已修复（2026-07-28）

- **`_typeset_fixed.html` 与 `_typeset.html`**：新管线只生成 `_typeset.html`；本地已删除已确认逐字节相同的旧重复文件。
- **`<title>` 哈希文件名**：Web 端会把用户上传的原始文件名写入高保真 HTML 标题。
- **浮点精度过剩**：已统一保留三位小数。
