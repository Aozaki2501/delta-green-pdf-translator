...
- 高保真排版支持网页一键修复：Chromium 检查失败时可直接点“修复溢出文字并重新检查”，只重译溢出的块，不整本重翻；并新增布局问题报告、修复清单和高保真质量检查报告。
...
| `_typeset_layout_issues.json` | 布局问题报告：浏览器实测各块溢出位置和像素 |
| `_typeset_layout_repair_targets.json` | 布局修复清单：溢出块的可定位修复目标 |
| `_typeset_quality.md` / `_typeset_quality.json` | 高保真质量检查：缺译文、疑似未翻、疑似截断等 |
