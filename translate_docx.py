...
Usage:
    ...
    python translate_docx.py input.docx --api-key YOUR_KEY --model deepseek-v4-pro --workers 8
...
def build_docx_progress_metadata(docx_path: str, glossary_path: str | None,
    ...
    return {
        ...
        "translate_headers": bool(translate_headers),
    }
...
def translate_docx_file(
    ...
    # Setup progress tracker. Named after the output file so two documents
    # written to the same folder no longer share (and wipe) one progress file.
    progress_file = str(Path(output_path).with_suffix("")) + ".progress.json"
    metadata = build_docx_progress_metadata(
        docx_path, glossary_path, model, base_url, fuzzy_matching, translate_headers
    )
